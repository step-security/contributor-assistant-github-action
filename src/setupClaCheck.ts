...
import { ClafileContentAndSha, CommitterMap, CommittersDetails, ReactedCommitterMap } from './interfaces'
...
async function createClaFileAndPRComment(
  ...
  committerMap.notSigned = committers.filter(committer => !!committer.id)
  committerMap.signed = []
...
function prepareCommiterMap(
  ...
  committerMap.notSigned = committers.filter(
    committer =>
      committer.id && !claFileContent?.signedContributors.some(cla => committer.id === cla.id)
  )
